...
import { Upload, X, FileText, Image, Check, AlertTriangle } from "lucide-react";
import { validateFile, formatFileSize, ALLOWED_FILE_TYPES, MAX_FILE_SIZE } from "@/lib/validators";
          ...
          >
            ...
            <div>
              <p className="text-sm font-medium text-navy-700">
                اسحب الملف هنا أو <span className="text-gold-600">اضغط للتصفح</span>
              </p>
              <p className="text-xs text-navy-400 mt-1">
                JPG, PNG, WebP, PDF • الحد الأقصى {MAX_FILE_SIZE / 1024 / 1024}MB
              </p>
            </div>
          </motion.div>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-navy-500 mt-0.5">
                {value.type || "غير معروف"} • {formatFileSize(value.size)}
              </p>
              <div className="flex items-center gap-1.5 mt-1">
                <Check className="w-3.5 h-3.5 text-emerald-500" />
                <span className="text-xs text-emerald-600">تم رفع الملف</span>
              </div>
            </div>
